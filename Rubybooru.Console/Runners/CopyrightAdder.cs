...
namespace Rubybooru.Console.Runners
{
    public class CopyrightAdder
    {
        ...
        public int Run(CopyrightAdderOptions options)
        {
            ...
            {
                if (options.BuilderVersion == 1)
                    return MapCreatorV1.Create(options.RawImageData, options.CopyrightMapPath, options.PercentageLimit);
                return MapCreatorV2.Create(options.RawImageData, options.CopyrightMapPath, options.PercentageLimit);
            }
